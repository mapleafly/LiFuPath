<template>
  <Dialog v-model="importDialogVisible" title="高校专业目录导入" width="400">
    <el-upload
      ref="importUploadRef"
...
import { MajorDirectoryApi } from '@/api/lfpath/majordirectory'
...
const importUrl =
  import.meta.env.VITE_BASE_URL + import.meta.env.VITE_API_URL + '/lfpath/major-directory/import'
...
const submitImportFormSuccess = (response: any) => {
  ...
  let text = '上传成功数量：' + data.createMajorNames.length + ';'
  text += '更新成功数量：' + data.updateMajorNames.length + ';'
...
const importTemplate = async () => {
  const res = await MajorDirectoryApi.importMajorDirectoryTemplate()
  download.excel(res, '高校专业目录导入模版.xls')
}
