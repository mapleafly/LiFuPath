<template>
  <Dialog v-model="importDialogVisible" title="本科普通批投档线信息导入" width="400">
    <el-upload
      ref="importUploadRef"
...
import { UndergraduateLowestScoreApi } from '@/api/lfpath/undergraduatelowestscore'
import { getAccessToken, getTenantId } from '@/utils/auth'
import download from '@/utils/download'

defineOptions({ name: 'UndergraduateLowestScoreImportForm' })
...
const importUrl =
  import.meta.env.VITE_BASE_URL + import.meta.env.VITE_API_URL + '/lfpath/undergraduate-lowest-score/import'

const uploadHeaders = ref() // 上传 Header 头
const importFileList = ref([]) // 导入文件列表
const importUpdateSupport = ref(0) // 是否更新已经存在的信息数据
...
const submitImportFormSuccess = (response: any) => {
  ...
  let text = '上传成功数量：' + data.createRecords.length + ';'
  text += '更新成功数量：' + data.updateRecords.length + ';'
  text += '更新失败数量：' + Object.keys(data.failureDetails).length + ';'
...
const importTemplate = async () => {
  const res = await UndergraduateLowestScoreApi.importUndergraduateLowestScoreTemplate()
  download.excel(res, '本科普通批投档线信息导入模版.xls')
}
